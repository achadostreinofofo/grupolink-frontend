...
function CapacityBar({ pct }: { pct: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 bg-night-700 rounded-full h-2">
        <div className={`${color} h-2 rounded-full`} style={{ width: `${p}%` }} />
      </div>
      <span className="text-xs text-night-300 w-9 text-right">{Math.round(p)}%</span>
    </div>
  )
}
...
function AnalyticsContent() {
  ...
  const stats = overview ? [
    ...
    { label: 'Cliques Total',    value: overview.totalClicks,                 icon: TrendingUp,        color: 'text-night-200 bg-night-700' },
  ] : []
  ...
  return (
    <div>
          ...
          {([7, 30] as const).map(d => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                days === d ? 'bg-brand-600 text-white' : 'bg-night-700 border border-night-600 text-night-200 hover:bg-night-700'
              }`}
            >
              {d}d
            </button>
          ...
          {stats.map(({ label, value, icon: Icon, color }) => (
            <Card key={label}>
              <CardContent className="flex items-center gap-3 py-3">
                ...
                <div>
                  <p className="text-lg font-bold text-night-50 leading-tight">{value.toLocaleString('pt-BR')}</p>
      ...
      <div>
        <h2 className="text-base font-semibold text-night-200 mb-4">Detalhes por Estrutura</h2>
        {structures.length === 0 ? (
          <p className="text-sm text-night-400">Nenhuma estrutura encontrada.</p>
        ) : (
...
function StructureCard({ structureId, structureName, days }: {
  ...
  if (!data) return <div className="h-48 bg-night-700 rounded-xl animate-pulse" />
      ...
      <CardHeader>
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="flex-1">
            <p className="font-semibold text-night-50">{structureName}</p>
            <p className="text-xs text-night-400 font-mono mt-0.5">{data.smartLink}</p>
          </div>
          <div className="flex gap-3 text-sm text-night-300 flex-wrap">
            <span><strong className="text-night-50">{data.totalClicks.toLocaleString('pt-BR')}</strong> cliques</span>
            <span><strong className="text-night-50">{data.totalMembers}</strong> membros</span>
          ...
          {tabs.map(t => (
            <button
              ...
              className={`px-3 py-1 rounded-lg text-xs font-medium transition-colors ${
                tab === t.key ? 'bg-brand-600 text-white' : 'text-night-300 hover:bg-night-700'
              }`}
            >
              {t.label}
            </button>
      ...
      <CardContent>
        {tab === 'clicks' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
              <p className="text-xs text-night-300 font-medium mb-3">Cliques por dia</p>
              <ClicksChart data={data.clicksByDay} days={days} />
            </div>
            <div>
              <p className="text-xs text-night-300 font-medium mb-3">Grupos ({data.groups.length})</p>
              <div className="space-y-3">
                {data.groups.map(g => (
                  <div key={g.groupId}>
                    <div className="flex items-center justify-between text-sm mb-1">
                      <span className="font-medium text-night-100 truncate">{g.groupName}</span>
                      <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                        <span className="text-xs text-night-400">{g.memberCount}/{g.maxMembers}</span>
            ...
            <div>
              <div className="flex gap-4 mb-4">
                ...
                <div className="bg-green-50 rounded-xl p-3 flex-1 text-center">
                  ...
                  <p className="text-xs text-night-400">Entradas (30d)</p>
                </div>
                <div className="bg-orange-50 rounded-xl p-3 flex-1 text-center">
                  ...
                  <p className="text-xs text-night-400">Taxa churn</p>
                </div>
              </div>
              <p className="text-xs text-night-300 font-medium mb-2">Saídas por dia</p>
              <ChurnChart data={churn.exitsByDay} />
            </div>
            <div>
              <p className="text-xs text-night-300 font-medium mb-3">Saídas por grupo</p>
              {churn.exitsByGroup.length === 0 ? (
                <p className="text-xs text-night-400 italic">Nenhuma saída registrada</p>
              ) : (
                <div className="space-y-2">
                  {churn.exitsByGroup.map(g => (
                    <div key={g.groupId} className="flex items-center justify-between text-sm py-1.5 border-b border-gray-50">
                      <span className="text-night-200 truncate">{g.groupName}</span>
